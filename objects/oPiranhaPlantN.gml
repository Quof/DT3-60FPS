...
if global.gamePaused=false
{
  ...
  if bActive=true and stunnedTime=0
  {
    ...
    if atkProg<=66 {atkProg+=1/FPS_SCA}
    else
    {
      findTargetX=point_distance(oPlayer1.x,0,x,0)
      if findTargetX>=36 {atkProg+=1/FPS_SCA}
    }

    if atkProg>=11 and atkProg<=26 {y-=2/FPS_SCA}
    else if atkProg>=51 and atkProg<=66 {y+=2/FPS_SCA}
