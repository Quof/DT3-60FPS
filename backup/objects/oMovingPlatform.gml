...
scaled_start=(((1-1)*FPS_SCA)+1) //QWH
scaled_loopend=((20)*FPS_SCA)
scaled_trueend=scaled_loopend+1
...
x+=round(xDir)
y+=round(yDir)
shiftTimeMax=shiftChange-2/FPS_SCA
...
if global.gamePaused=false
{
  if bCanMove=true
  {
    if shiftTime>=shiftTimeMax
    {
      ...
      else if shiftSeq>=scaled_start and shiftSeq<=scaled_loopend
        shiftSeq+=1
      else if shiftSeq>=scaled_trueend
      {
        shiftTime=0
        shiftSeq=0
        xVel=xDir
